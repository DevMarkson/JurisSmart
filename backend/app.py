                ...
                if citations:  
                    ...
                    for title in doc_to_title_map.values():  
                        if title not in unique_citations:  
                            pdf_link = pdf_links_dict.get(title, "No link available")  
                            unique_citations[title] = {'number': citation_count, 'title': title, 'pdf_link': pdf_link}  
                            citation_count += 1  
                    ...
                    for doc_placeholder, title in doc_to_title_map.items():  
                        if title in unique_citations:  
                            ...
                            response_text = response_text.replace(f"[{doc_placeholder}]", f"[{citation_number}]")  
